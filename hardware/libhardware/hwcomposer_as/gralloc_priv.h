...
#define GRALLOC_ARM_NUM_FDS 0
...
#if GRALLOC_ARM_UMP_MODULE
	private_handle_t(int flags,int usage, int size, int base, int lock_state, ump_secure_id secure_id, ump_handle handle,  int paddr):
#if GRALLOC_ARM_DMA_BUF_MODULE
		share_fd(-1),
#endif
		magic(sMagic),
...
#if GRALLOC_ARM_DMA_BUF_MODULE
	private_handle_t(int flags, int usage, int size, int base, int lock_state,  int paddr):
		share_fd(-1),
		magic(sMagic),
		flags(flags),
		usage(usage),
	...
	private_handle_t(int flags,int usage,int size, int base, int lock_state, int fb_file, int fb_offset, int paddr):
#if GRALLOC_ARM_DMA_BUF_MODULE
		share_fd(-1),
#endif
		magic(sMagic),

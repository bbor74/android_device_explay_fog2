...
static int _hwcdev_setup_layer(SUNXI_hwcdev_context_t *ctx, __disp_layer_info_t *layer_info, SUNXI_hwcdev_pipe_t *psPipe, size_t disp)
{
    ...
    if(_hwcdev_is_scaled(layer) || handle->format==HAL_PIXEL_FORMAT_YV12)
    {
		...
		if ((layer_info->scn_win.x + layer_info->scn_win.width > ctx->hdmi_valid_width) && (disp == 1)) {
			layer_info->scn_win.x -= (layer_info->scn_win.x + layer_info->scn_win.width - ctx->hdmi_valid_width);
		}
